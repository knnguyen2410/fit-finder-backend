...
public class JWTUtils {
    ...
    /**
     * generateJwtToken uses the owner's details (email address) to create a jwt token.
     * This method runs once after the user is authenticated.
     * @param myOwnerDetails email address is used
     * @return a json web token for the user
     */
    public String generateJwtToken(MyOwnerDetails myOwnerDetails) {
        return Jwts.builder()
                .setSubject((myOwnerDetails.getUsername())) // gets email address only (keep info to a minimum)
